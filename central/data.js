...
// Code has been modified to split up the data for distribution to edge servers.
...
class MnistDataset {
  ...
  async getTrainData(edges) {
    return await this.getData_(true, edges);
  }

  async getTestData(edges) {
    return await this.getData_(false, edges);
  }

  async tensorSerializer(tensor, s){
    const data = await tensor.data();
    const tb64 = Buffer.from(new Uint8Array(data.buffer)).toString('base64');
    return {data: tb64, shape: s, ttype: typeof data};
  }

  async tensorUnSerializer(sdata){
    const raw = Uint8Array.from(atob(sdata.data), c => c.charCodeAt(0));
    return tf.tensor(raw, sdata.shape, sdata.ttype);
  }

  async getData_(isTrainingData, edges) {
    let imagesIndex;
    let labelsIndex;
    if (isTrainingData) {
      imagesIndex = 0;
    ...
    let numClient = 0;
    for (let ed in edges) numClient += edges[ed].numClients;
    const perClient = Math.floor(size/numClient);
    //const perClient = 4;
    ...
    const cshape = [perClient, IMAGE_HEIGHT * IMAGE_WIDTH];
    ...
    for (let ed in edges) {
      ...
      for (let i = 0; i < edge.numClients; i++){
        temp.push({
          images: await this.tensorSerializer(tf.tensor2d(images.slice(ind*IMAGE_FLAT_SIZE, (ind+perClient)*IMAGE_FLAT_SIZE), cshape), cshape),
          labels: await this.tensorSerializer(tf.oneHot(tf.tensor1d(labels.slice(ind, ind+perClient), 'int32'), LABEL_FLAT_SIZE).toFloat(), [perClient, LABEL_FLAT_SIZE]),
        })
        ind += 1;
      }
      out.push(temp);
