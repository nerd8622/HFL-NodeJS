 ...
 * =============================================================================
 */
// Modified by David Canaday to output to custom file format.
...
const genTrainData = async () => {
  const imgpath = path.join(__dirname, "model/img.bin");
  const lblpath = path.join(__dirname, "model/lbl.bin");
  ...
  [TRAIN_IMAGES_FILE,TRAIN_LABELS_FILE,TEST_IMAGES_FILE,TEST_LABELS_FILE].forEach((file) => {
    fs.unlink(pathConvert(file), err => {});
  });
}
